...
namespace Panosen.CodeDom.Vue.Engine
{
    ...
    partial class VueCodeScriptEngine
    {
        ...
        /// GenerateCreated
        /// </summary>
        public void GenerateCreated(CodeMethod codeMethod, CodeWriter codeWriter, GenerateOptions options = null)
        {
            if (codeMethod == null) { return; }
            if (codeWriter == null) { return; }
            options = options ?? new GenerateOptions();

            codeWriter.Write(options.IndentString).Write(KEYWORD_CREATED).Write(Marks.COLON).Write(Marks.WHITESPACE);
